...
"""
文件名：image_check.py
功能：桥梁表观病害智能检测（支持 YOLOv8 目标检测 + 基于检测框的近似量化）
输入：桥梁表面图像路径
输出：病害检测结果（类别+置信度+定位信息+近似长度宽度面积）
"""
...
def bbox_quantize(xyxy, pixel_to_mm):
    """
    基于检测框做近似量化：
    - 长度 = 长边
    - 宽度 = 短边
...
def yolo_detect(image_path, conf_threshold=0.25, iou_threshold=0.45, max_det=100):
    ...
    try:
        ...
        if boxes is not None:
            for box in boxes:
                ...
                quant = bbox_quantize(xyxy, Config.pixel_to_mm)

                detections.append({
                    'class': cls_name,
                    'class_cn': Config.label_map_cn.get(cls_name, cls_name),
                    'confidence': conf,
    ...
    prediction = best_pred['class'] if best_pred else '未检测到病害'
...
def check_handle(image_path):
    ...
    current_model_path = get_yolo_model_path()
    if _HAS_YOLO and os.path.exists(current_model_path):
        try:
            return yolo_detect(image_path)
        except Exception as e:
            print(f"YOLO检测失败（{current_model_path}），回退到分类模型：{e}")
